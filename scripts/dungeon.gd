...
func generate_rewards(dungeon_tier: int, adventurer_level: int = 1) -> Dictionary:
	...
	# Temp: ore drops (later provided by GameManager)
	...
	var base_exp = 50 + dungeon_tier * 20  # Base EXP
...
## Random ore for dungeon difficulty
func _get_random_ore_for_tier(tier: int) -> String:
	var ores = ["copper", "tin", "iron", "silver", "gold", "mithril", "orichalcum"]
	var available_ores = []
	
	for ore in ores:
		# Adaptive: higher difficulty can still drop lower ores, but higher ores more frequent
		var min_tier = 1
		match ore:
