...
func _update_list() -> void:
	...
	for i in range(items.size()):
		...
		price_label.text = "[GOLD]%d" % item["price"]
		...
		# Normal item or artifact (equippable)
...
func _on_sell_item(item: Dictionary) -> void:
	...
	if price > 0:
		sell_result.text = "[GOLD] Earned %d Gold!" % price
		_flash_result()
		_update_list()
...
func _on_sell_all() -> void:
	...
	if total > 0:
		sell_result.text = "[GOLD] Earned %d Gold total!" % total
		_flash_result()
		_update_list()


func _on_equip_item(inventory_index: int) -> void:
	# Adventurer selection popup (simple - first adventurer)
	var adventurers = GameManager.get_adventurers()
	if adventurers.is_empty():
		sell_result.text = "[WARN] No adventurers available!"
		_flash_result()
		return
	...
	if success:
		...
		sell_result.text = "[OK] Equipped %s to %s!" % [inv_items[inventory_index]["name"] if inventory_index < inv_items.size() else "item", adv.name]
		_flash_result()
		_update_list()
	else:
		sell_result.text = "[X] Equip failed!"
		_flash_result()
